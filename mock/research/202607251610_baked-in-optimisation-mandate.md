...
**Corrected by that sketch (`sketches/202607252030_lease-masks-are-escape-not-uniqueness.md`): the inverse reading FAILS.** `reach` is joined up the tree, so every ancestor of a use site inside the binder's scope carries the binder's slot, and counting nodes whose mask contains a slot counts an ancestor chain rather than the places that hold the value. The deeper error is a category one: reach is about escape, uniqueness at an update site is about last use, and last use is a dataflow property over an ordering rather than a set closed over a tree. What survives is a two-part shape: reach is a sound escape filter and is a genuine necessary half already computed, and last use is a new backward scan whose cost ANF has already paid down, since a normalised body is a linear chain of bindings where the last occurrence is the last use. The paragraph below on the slot collision still holds, and holds for the escape filter: a collision can only make a value look like it escapes when it does not, which refuses a safe reuse and never permits an unsafe one.
