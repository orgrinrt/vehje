...
## Open design question, resolve before impl: is a block a linear instruction stream or a tree region?

Surfaced while scoping the implementation. A `Block.nodes` `NodeRange` is a span, but the execution model over that span is not yet fixed, and it decides both the node ordering the serializer emits and the Zig interpreter's shape:

- **Linear instruction stream.** Each node in the block's span is an instruction executed in order; ANF's named intermediates are the instruction results, held in a per-frame slot array indexed by binder. The interpreter is a straight loop over `nodes[start..start+len]`, a `dispatch`-style switch (the `zig-dispatch/dispatch.zig` bench is the reference). The serializer must emit the span in execution order (operands before their consumer), which is a topological flatten of the block's ANF sub-DAG. This is the classic bytecode-ish shape and is what "maximal straight-line region" most naturally means.
- **Tree region.** The span is the set of nodes in the block, and the interpreter tree-walks from the block's tail following `NodeRef` children (the `carrier-zig/interp.zig` recursive-eval bench is the reference), stopping at block boundaries. No linearization; the order in the span is only for locality. Simpler to produce (no topological flatten) but a recursive walk, and it blurs the "instruction stream" the tier model implies for the bytecode tier.

The two imply different serializers and different `runtime.zig` execution cores, so the choice is load-bearing and must be made before `lower_to_cfg` + the Zig interpreter are written. The likely answer is the linear stream (it matches "straight-line region", the slot-array frame the effect/resumption model wants, and the bytecode tier's direction), but it carries the topological-flatten obligation on the serializer, and it is op's call given it shapes the whole runtime. The `Tier::Arena` reference semantics can start with the tree-walk (cheaper to stand up, proves the residual decodes and runs) and the linear stream can be the `Tier::Bytecode` form, which is one clean reading of the two tiers; that framing is itself the proposal to confirm. This question, plus the node-re-serialization coupling above, are the two the next design pass (or op) settles before the impl round opens.
