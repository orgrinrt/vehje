...
## Addendum: two findings from reading the Core, which sharpen "what is owed"

Working the mapping against `vehje-ir/src/node.rs` rather than against memory turns two of the owed items above from "unbuilt" into something more specific, and one of them is a design decision that this study has just made due.

**The pattern representation is undecided, not merely unimplemented.** `Match` carries an explicit note: the arms are stored as a plain node list, the pattern representation is deferred, and it lands "with the first matching consumer" because no consumer matched when the Core was cut. This study is that consumer. So pattern matching is blocked on a Core design round that decides how a pattern is represented (what the pattern node kinds are, how a binding pattern introduces its binder, how exhaustiveness is expressed to the checker, and whether guards are patterns or a separate arm field), and only then on runtime evaluation. `Handle` carries the same shape of deferral for its clauses, awaiting the bounded-multi-shot continuation encoding. Neither is an oversight; both are honest deferrals whose trigger has now fired.

**Compound values enter through the introduction projection, not through a Core form.** The twelve forms include `Project`, which reads a field, and `Iter`, which folds a sequence, but nothing that builds a record or a sequence. That is not a gap. It follows from the settled position that content-as-values is the signature's introduction projection: the Core is the eliminator algebra, and construction belongs to a family. So a struct literal in the surface lowers to a family operation that produces a record, exactly as arithmetic lowers to one that produces an integer, and `Project` eliminates what the family introduced.

The consequence is worth stating plainly, because it reorders the owed list. A record cannot be produced by any program until a family handler can *return* one, and a string cannot be operated on until a handler can *receive* one. Both are the compound-operand escape that the family-dispatch round named and deferred. That single extension point, rather than `Project` or `Match` evaluation, is the real binding constraint on the language becoming useful: with it, records and strings exist and `Project` becomes implementable; without it, the runtime's value set cannot grow no matter which Core forms gain evaluation.

So the sharpened order of what the framework owes this consumer is: the compound operand and return form across the host boundary, which is what lets values become richer than scalars; then a runtime value arena to hold what a handler returns; then `Project` evaluation over records; then the Core's pattern-representation round, and `Match` evaluation after it. Traits, generics, and monomorphisation need none of this and can proceed in the front end in parallel, since they erase before anything is emitted.
