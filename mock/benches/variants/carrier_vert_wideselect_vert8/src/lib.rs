...
fn run<const N: usize>(input: &[u8; N], output: &mut [u8; 8]) -> FfiBenchCall {
    ...
    timed_calibrated! { run {
        ...
        while k < ITERS {
            ...
            let mut s = [0u64; 8]; for (l, x) in s.iter_mut().enumerate() { *x = seed ^ (l as u64).wrapping_mul(0x9e37_79b9); } acc ^= c::vertical::interpret_vertical_checksum_into::<8>(pd, &s, &mut soa);
            k += 1;
        }
