...
fn run<const N: usize>(input: &[u8; N], output: &mut [u8; 8]) -> FfiBenchCall {
    ...
    static PREP: OnceLock<Vec<u8>> = OnceLock::new(); let bytes = PREP.get_or_init(|| { let mut gp = c::GenParams::profile("wideselect").unwrap(); gp.node_count = N; c::ir::encode(&c::generate(&gp), &c::ir::REC24) });
    ...
    timed_calibrated! { run {
        ...
        while k < ITERS {
            ...
            let d = c::ir::Decoded::parse(bytes, c::ir::REC24).unwrap(); acc ^= d.node_count as u64;
            k += 1;
        }
