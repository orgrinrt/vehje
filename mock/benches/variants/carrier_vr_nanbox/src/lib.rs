...
#[bench_variant("carrier_vr_nanbox", sizes = [64, 256, 1024, 4096, 16384])]
fn run<const N: usize>(input: &[u8; N], output: &mut [u8; 8]) -> FfiBenchCall {
    let _ = &input; // some cells (native_ceiling) sweep `input`; others use SEEDS.
    static PREP: OnceLock<Vec<c::valrepr::VNode>> = OnceLock::new(); let prog = PREP.get_or_init(|| c::valrepr::gen_valprog(N, 0x1234_5678)); let mut vals = vec![0u64; prog.len()];
    ...
    timed_calibrated! { run {
        ...
        while k < ITERS {
            ...
            acc ^= c::valrepr::interp_nanbox(prog, seed as i32, &mut vals);
            k += 1;
        }
