...
// REAL tree-walking interpreter: dispatch every node. Match handled by strategy `predicate`.
fn eval(nd: &[Node], a: &[u32;4], node: u32, b: u8) -> u64 {
    ...
    match n.op {
        ...
        KEYOP => key_fn(b),
        MATCH => { let k = eval(nd,a,n.a,b); let mut v=[0u64;4]; let mut i=0usize; while i<4 { v[i]=eval(nd,a,a[i],b); i+=1; } v[k as usize] }
        _ => 0,
    }
}
// SHOWDOWN-IR interp tier: strategy `predicate`
#[bench_variant("ir_predicate_int", sizes = [64, 256, 1024, 4096, 16384])]
