...
#[bench_variant("carrier_lay_scatter_rec20", sizes = [64, 256, 1024, 4096, 16384])]
fn run<const N: usize>(input: &[u8; N], output: &mut [u8; 8]) -> FfiBenchCall {
    let _ = &input; // some cells (native_ceiling) sweep `input`; others use SEEDS.
    static PREP: OnceLock<Vec<u8>> = OnceLock::new(); let bytes = PREP.get_or_init(|| { let mut gp = c::GenParams::profile("scatter").unwrap(); gp.node_count = N; c::ir::encode(&c::generate(&gp), &c::ir::REC20) }); let d = c::ir::Decoded::parse(bytes, c::ir::REC20).unwrap(); let mut r = vec![0u64; d.node_count];
