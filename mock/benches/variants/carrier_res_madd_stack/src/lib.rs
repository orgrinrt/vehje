...
#[bench_variant("carrier_res_madd_stack", sizes = [64, 256, 1024, 4096, 16384])]
fn run<const N: usize>(input: &[u8; N], output: &mut [u8; 8]) -> FfiBenchCall {
    let _ = &input; // some cells (native_ceiling) sweep `input`; others use SEEDS.
    static PREP: OnceLock<c::stackbc::StackProgram> = OnceLock::new(); let sp = PREP.get_or_init(|| { let mut gp = c::GenParams::profile("madd").unwrap(); gp.node_count = N; c::stackbc::compile(&c::generate(&gp)) }); let mut st = vec![0u64; 64]; let mut lo = vec![0u64; sp.num_locals];
