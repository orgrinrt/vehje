...
#[inline(always)] fn program(b:u64)->u64 { 0u64 ^ { let k=key4(b); let arr=[lin(b,0), lin(b,1), lin(b,2), lin(b,3)]; arr[k as usize] } ^ { let k=key4(b); let arr=[nm(b,0), nm(b,1), nm(b,2), nm(b,3)]; arr[k as usize] } ^ { let k=(b & 7); let arr=[lin(b,0), lin(b,1), lin(b,2), lin(b,3), lin(b,4), lin(b,5), lin(b,6), lin(b,7)]; arr[k as usize] } ^ { let k=key4(b); let arr=[blk(b,0), blk(b,1), blk(b,2), blk(b,3)]; arr[k as usize] } ^ { let bk=((b>=50) as u64+(b>=120) as u64+(b>=200) as u64); match bk { 0=>lin(b,0), 1=>lin(b,1), 2=>lin(b,2), _=>lin(b,3) } } ^ { let bk=((b>=50) as u64+(b>=120) as u64+(b>=200) as u64); match bk { 0=>ni(b,0), 1=>ni(b,1), 2=>ni(b,2), _=>ni(b,3) } } ^ { let bk=((b>=50) as u64+(b>=120) as u64+(b>=200) as u64); match bk { 0=>blk(b,0), 1=>blk(b,1), 2=>blk(b,2), _=>blk(b,3) } } }
#[bench_variant("an_match_pred", sizes = [64, 256, 1024, 4096, 16384])]
