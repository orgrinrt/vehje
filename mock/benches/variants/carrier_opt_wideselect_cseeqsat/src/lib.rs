...
#[bench_variant("carrier_opt_wideselect_cseeqsat", sizes = [64, 256, 1024, 4096, 16384])]
fn run<const N: usize>(input: &[u8; N], output: &mut [u8; 8]) -> FfiBenchCall {
    static PREP: OnceLock<(Vec<u8>, Vec<u32>)> = OnceLock::new(); let (bytes, out_ids) = PREP.get_or_init(|| { let mut gp = c::GenParams::profile("wideselect").unwrap(); gp.node_count = N; let prog = c::generate(&gp); let opt = c::optimize::optimize(&prog, true, false, false, true); (c::ir::encode(&opt.prog, &c::ir::REC24), opt.out_ids) }); let d = c::ir::Decoded::parse(bytes, c::ir::REC24).unwrap(); let mut r = vec![0u64; d.node_count.max(1)];
