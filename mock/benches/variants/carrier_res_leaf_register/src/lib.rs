...
#[bench_variant("carrier_res_leaf_register", sizes = [64, 256, 1024, 4096, 16384])]
fn run<const N: usize>(input: &[u8; N], output: &mut [u8; 8]) -> FfiBenchCall {
    let _ = &input; // some cells (native_ceiling) sweep `input`; others use SEEDS.
    static PREP: OnceLock<(c::predecode::Predecoded, Vec<u32>)> = OnceLock::new(); let (pd, sink_ids) = PREP.get_or_init(|| { let mut gp = c::GenParams::profile("leaf").unwrap(); gp.node_count = N; let prog = c::generate(&gp); let sinks = c::optimize::sinks(&prog); let bytes = c::ir::encode(&prog, &c::ir::REC24); let d = c::ir::Decoded::parse(&bytes, c::ir::REC24).unwrap(); (c::predecode::predecode(&d), sinks) }); let mut r = vec![0u64; pd.nodes.len()];
