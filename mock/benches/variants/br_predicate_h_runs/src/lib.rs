...
// strategy `predicate`, arm-cost `h` (W=24), situation `runs`: correlated long runs (flip when b<24)
#[bench_variant("br_predicate_h_runs", sizes = [64, 256, 1024, 4096, 16384])]
fn run<const N: usize>(input: &[u8; N], output: &mut [u8; 8]) -> FfiBenchCall {
    timed! {
        run {
            ...
            let mut prev = false;
            for &b in input.iter() {
                if b < 24 { prev = !prev; } let cond = prev;
                let t = arm_taken(b, W); let nt = arm_nottaken(b, W); let v = if cond { t } else { nt };
                acc ^= v;
            }
