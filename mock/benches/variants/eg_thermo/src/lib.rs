...
// thermo encoding. Effects are computed once per subprocess (from the fixed DAG)
...
fn compute_eff(dag: &[Node]) -> Vec<u64> {
    ...
    for i in 0..dag.len() {
        let nd = &dag[i];
        eff[i] = if nd.op == 0 { (if nd.grade == 2 { 0b11u64 } else { 0b01u64 }) << (nd.fam as u64 * 2) } else { eff[nd.a as usize] | eff[nd.b as usize] };
    }
    eff
}
...
#[bench_variant("eg_thermo", sizes = [64, 256, 1024, 4096, 16384])]
fn run<const N: usize>(input: &[u8; N], output: &mut [u8; 8]) -> FfiBenchCall {
    let eff = EFF.get_or_init(|| compute_eff(&gen_dag(N)));
    let mut base: u64 = 0;
    {
        let mut f = 0u64;
        while f < PERMIT { base |= 0b11u64 << (f * 2); f += 1; }
    }
    timed! { run {
        let mut acc: u64 = 0;
        for k in 0..ITERS {
            ...
            let mask = 3u64 << (tf * 2);
            let target = (base & !mask) | (((1u64 << tg) - 1) << (tf * 2));
            let mut viol: u64 = 0;
            for &e in eff.iter() {
                if (e & !target) != 0 { viol += 1; }
            }
