...
// strategy `branch`, arm-cost `h` (W=24), situation `runs`: correlated long runs (flip when b<24)
#[bench_variant("br_branch_h_runs", sizes = [64, 256, 1024, 4096, 16384])]
    ...
    timed! {
        run {
            ...
            for &b in input.iter() {
                ...
                let v = if cond { arm_taken(b, W) } else { arm_nottaken(b, W) };
                acc ^= v;
            }
