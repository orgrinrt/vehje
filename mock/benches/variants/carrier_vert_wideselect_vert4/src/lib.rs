...
#[bench_variant("carrier_vert_wideselect_vert4", sizes = [64, 256, 1024, 4096, 16384])]
fn run<const N: usize>(input: &[u8; N], output: &mut [u8; 8]) -> FfiBenchCall {
    let _ = &input; // some cells (native_ceiling) sweep `input`; others use SEEDS.
    static PREP: OnceLock<c::predecode::Predecoded> = OnceLock::new(); let pd = PREP.get_or_init(|| { let mut gp = c::GenParams::profile("wideselect").unwrap(); gp.node_count = N; let bytes = c::ir::encode(&c::generate(&gp), &c::ir::REC24); let d = c::ir::Decoded::parse(&bytes, c::ir::REC24).unwrap(); c::predecode::predecode(&d) }); let mut soa = c::vertical::make_scratch::<4>(pd.nodes.len());
