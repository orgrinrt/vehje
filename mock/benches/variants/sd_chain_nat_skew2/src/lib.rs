...
// SHOWDOWN native tier: strategy `chain`, scenario `skew2`
#[bench_variant("sd_chain_nat_skew2", sizes = [64, 256, 1024, 4096, 16384])]
fn run<const N: usize>(input: &[u8; N], output: &mut [u8; 8]) -> FfiBenchCall {
    timed! { run {
        let mut acc: u64 = 0;
        for &b in input.iter() {
            let k: u32 = if b < 179 { 2 } else { [0u32,1,3][(b as usize) % 3] };
            let idx: u32 = if k == 0 { 0 } else if k == 1 { 1 } else if k == 2 { 2 } else { 3 };
            let v = arm(idx, b, W);
            acc ^= v;
        }
