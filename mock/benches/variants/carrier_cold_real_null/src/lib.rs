...
#[bench_variant("carrier_cold_real_null", sizes = [64, 256, 1024, 4096, 16384])]
fn run<const N: usize>(input: &[u8; N], output: &mut [u8; 8]) -> FfiBenchCall {
    let _ = &input; // some cells (native_ceiling) sweep `input`; others use SEEDS.
    static PREP: OnceLock<Vec<c::predecode::Predecoded>> = OnceLock::new(); let pds = PREP.get_or_init(|| { (0..16u64).map(|i| { let mut gp = c::GenParams::profile("real").unwrap(); gp.node_count = N; gp.seed ^= i.wrapping_mul(0x9e37_79b9_7f4a_7c15); let bytes = c::ir::encode(&c::generate(&gp), &c::ir::REC24); let d = c::ir::Decoded::parse(&bytes, c::ir::REC24).unwrap(); c::predecode::predecode(&d) }).collect() }); let mut r = vec![0u64; pds.iter().map(|q| q.nodes.len()).max().unwrap()];
