...
#[bench_variant("carrier_ent_c900_wmax", sizes = [64, 256, 1024, 4096, 16384])]
fn run<const N: usize>(input: &[u8; N], output: &mut [u8; 8]) -> FfiBenchCall {
    let _ = &input; // some cells (native_ceiling) sweep `input`; others use SEEDS.
    static PREP: OnceLock<Vec<u8>> = OnceLock::new(); let bytes = PREP.get_or_init(|| { let mut gp = c::GenParams::default_point(); gp.node_count = N; gp.op_correlation = 900; gp.locality_window = 1usize << 20; c::ir::encode(&c::generate(&gp), &c::ir::REC24) }); let d = c::ir::Decoded::parse(bytes, c::ir::REC24).unwrap(); let pd = c::predecode::predecode(&d); let mut r = vec![0u64; d.node_count];
    ...
    timed_calibrated! { run {
        ...
        while k < ITERS {
            ...
            c::predecode::interpret_predecoded(&pd, seed, &mut r); acc ^= c::checksum(&r);
            k += 1;
        }
