...
// SHOWDOWN-IR native tier (IR lowered; reference ceiling): strategy `predicate`
#[bench_variant("ir_predicate_nat", sizes = [64, 256, 1024, 4096, 16384])]
fn run<const N: usize>(input: &[u8; N], output: &mut [u8; 8]) -> FfiBenchCall {
    timed! { run {
        let mut acc: u64 = 0;
        for &b in input.iter() { let k = key_fn(b); acc ^= { let mut v=[0u64;4]; let mut i=0u32; while i<4 { v[i as usize]=arm(i,b); i+=1; } v[k as usize] }; }
