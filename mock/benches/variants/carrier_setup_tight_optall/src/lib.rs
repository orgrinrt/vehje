...
#[bench_variant("carrier_setup_tight_optall", sizes = [64, 256, 1024, 4096, 16384])]
fn run<const N: usize>(input: &[u8; N], output: &mut [u8; 8]) -> FfiBenchCall {
    let _ = &input; // some cells (native_ceiling) sweep `input`; others use SEEDS.
    static PREP: OnceLock<c::ir::Program> = OnceLock::new(); let prog = PREP.get_or_init(|| { let mut gp = c::GenParams::profile("tight").unwrap(); gp.node_count = N; c::generate(&gp) });
