...
#[bench_variant("carrier_cfg_fntable", sizes = [64, 256, 1024, 4096, 16384])]
fn run<const N: usize>(input: &[u8; N], output: &mut [u8; 8]) -> FfiBenchCall {
    static B: OnceLock<Vec<c::cfg::Block>> = OnceLock::new(); let blocks = B.get_or_init(|| c::cfg::build_nested_loop(N as u64, 4));
    ...
    timed_calibrated! { run {
        ...
        while k < ITERS {
            ...
            let (rr, _, _) = c::cfg::interp_fntable(blocks, seed, u64::MAX); acc ^= rr;
            k += 1;
        }
