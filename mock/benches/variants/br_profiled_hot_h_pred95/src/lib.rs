...
// strategy `profiled_hot`, arm-cost `h` (W=24), situation `pred95`: ~95% taken, predictable (b<243)
#[bench_variant("br_profiled_hot_h_pred95", sizes = [64, 256, 1024, 4096, 16384])]
fn run<const N: usize>(input: &[u8; N], output: &mut [u8; 8]) -> FfiBenchCall {
    timed! {
        run {
            ...
            for &b in input.iter() {
                let cond = b < 243;
                let mut v = arm_taken(b, W); if !cond { v = arm_nottaken(b, W); }
                acc ^= v;
            }
