...
#[bench_variant("carrier_ceil_interp", sizes = [64, 256, 1024, 4096, 16384])]
fn run<const N: usize>(input: &[u8; N], output: &mut [u8; 8]) -> FfiBenchCall {
    static PREP: OnceLock<Vec<u8>> = OnceLock::new(); let bytes = PREP.get_or_init(|| c::madd_bytes(N / 4, c::ir::REC24)); let d = c::ir::Decoded::parse(bytes, c::ir::REC24).unwrap(); let mut r = vec![0u64; d.node_count];
    ...
    timed_calibrated! { run {
        ...
        while k < ITERS {
            ...
            for &b in input.iter() { c::interpret(&d, b as u64, &mut r); acc ^= c::checksum(&r); }
            k += 1;
        }
