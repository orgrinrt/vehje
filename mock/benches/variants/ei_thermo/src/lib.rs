...
// bottom-up lattice join over a fixed node DAG, thermo encoding. The DAG is
...
#[bench_variant("ei_thermo", sizes = [64, 256, 1024, 4096, 16384])]
    ...
    timed! { run {
        ...
        for k in 0..ITERS {
            ...
            for i in 0..N {
                let nd = &dag[i];
                eff[i] = if nd.op == 0 {
                    let mut e = (if nd.grade == 2 { 0b11u64 } else { 0b01u64 }) << (nd.fam as u64 * 2);
                    if i == 0 { e = e | ((if xg == 2 { 0b11u64 } else { 0b01u64 }) << (xf * 2)); }
                    e
                } else {
                    eff[nd.a as usize] | eff[nd.b as usize]
                };
            }
            let mut c = ib;
            for i in 0..N { c = c.wrapping_mul(0x0100_0000_01b3) ^ (eff[i].count_ones() as u64); }
            acc ^= c;
        }
