...
// REAL tree-walking interpreter: dispatch every node. Match handled by strategy `jumptable`.
fn eval(nd: &[Node], a: &[u32;4], node: u32, b: u8) -> u64 {
    ...
    match n.op {
        ...
        KEYOP => key_fn(b),
        MATCH => { let k = eval(nd,a,n.a,b); eval(nd,a,a[k as usize],b) }
        _ => 0,
    }
}
// SHOWDOWN-IR interp tier: strategy `jumptable`
#[bench_variant("ir_jumptable_int", sizes = [64, 256, 1024, 4096, 16384])]
