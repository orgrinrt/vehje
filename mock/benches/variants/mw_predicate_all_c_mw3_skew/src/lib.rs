...
// multiway `predicate_all`, arm-cost `c` (W=1), situation `mw3_skew`: 3-way, skewed to arm 0 (~80%)
#[bench_variant("mw_predicate_all_c_mw3_skew", sizes = [64, 256, 1024, 4096, 16384])]
fn run<const N: usize>(input: &[u8; N], output: &mut [u8; 8]) -> FfiBenchCall {
    timed! { run {
        ...
        for &b in input.iter() {
            ...
            let v;
            let all = [arm(0, b, W), arm(1, b, W), arm(2, b, W)]; v = all[k as usize];
            acc ^= v;
        }
