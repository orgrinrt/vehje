...
// both-known fast path lowers to a concrete multiply, else the loop. Operands (a mix ~80% both-known) are built once per subprocess from a
...
fn tmul(a: (u64, u64), b: (u64, u64)) -> (u64, u64) {
    if a.1 == 0 && b.1 == 0 { (a.0.wrapping_mul(b.0), 0) } else { tmul_loop(a, b) }
}

static OPS: OnceLock<Vec<(u64, u64)>> = OnceLock::new();
const ITERS: usize = 2;

#[bench_variant("tm_fastpath", sizes = [64, 256, 1024, 4096, 16384])]
