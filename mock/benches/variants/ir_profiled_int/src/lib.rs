...
// REAL tree-walking interpreter: dispatch every node. Match handled by strategy `profiled`.
fn eval(nd: &[Node], a: &[u32;4], node: u32, b: u8) -> u64 {
    ...
    match n.op {
        ...
        KEYOP => key_fn(b),
        MATCH => { let k = eval(nd,a,n.a,b); let arm = if k==2 {a[2]} else if k==0 {a[0]} else if k==1 {a[1]} else {a[3]}; eval(nd,a,arm,b) }
        _ => 0,
    }
}
// SHOWDOWN-IR interp tier: strategy `profiled`
#[bench_variant("ir_profiled_int", sizes = [64, 256, 1024, 4096, 16384])]
