...
// SHOWDOWN-IR native tier (IR lowered; reference ceiling): strategy `profiled`
#[bench_variant("ir_profiled_nat", sizes = [64, 256, 1024, 4096, 16384])]
fn run<const N: usize>(input: &[u8; N], output: &mut [u8; 8]) -> FfiBenchCall {
    timed! { run {
        let mut acc: u64 = 0;
        for &b in input.iter() { let k = key_fn(b); acc ^= { let idx=if k==2 {2} else if k==0 {0} else if k==1 {1} else {3}; arm(idx,b) }; }
