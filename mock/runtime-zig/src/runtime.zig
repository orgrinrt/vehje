...
pub const host_boundary = @import("host.zig");

// re-exported so the C ABI vocabulary stays reachable from one root
pub const VehjeOperand = host_boundary.VehjeOperand;
pub const VehjeHost = host_boundary.VehjeHost;
pub const SCALAR_UNIT = host_boundary.SCALAR_UNIT;
pub const SCALAR_BOOL = host_boundary.SCALAR_BOOL;
pub const SCALAR_INT = host_boundary.SCALAR_INT;
pub const SCALAR_STR = host_boundary.SCALAR_STR;
pub const COMPOUND_SEQ = host_boundary.COMPOUND_SEQ;
pub const COMPOUND_RECORD = host_boundary.COMPOUND_RECORD;
...
pub const Value = union(enum) {
    ...
    boolean: bool,
    int: i64,
    /// A string: the bytes themselves, wherever they live. A literal's point
    /// into the residual image and a handler's result into the session
    /// scratch; both outlive the evaluation.
    str: []const u8,
    /// A record or a sequence, by its index in the value arena. Unlike a string
    /// literal, a compound is not a slice into anything already lent, so it
    /// lives in the arena the host lends for the call.
    compound: u32,
    closure: Closure,
};
...
pub const EvalError = error{
    ...
    NoScratch, // a handler returned bytes with no session scratch to hold them
...
///
/// The scratch holds bytes a handler produced, which exist nowhere else: the
/// runtime allocates nothing, so the host lends the region and the handle's
/// lifetime is the lifetime a returned string needs.
pub const Session = struct {
    scratch: []u8,
    used: usize = 0,

    /// Copy `bytes` into the scratch, returning the copy.
    pub fn keep(self: *Session, bytes: []const u8) EvalError![]const u8 {
        if (self.used + bytes.len > self.scratch.len) return EvalError.ScratchFull;
        const at = self.used;
        @memcpy(self.scratch[at .. at + bytes.len], bytes);
        self.used += bytes.len;
        return self.scratch[at .. at + bytes.len];
    }
};
...
const Image = struct {
    ...
    /// The blob bytes a literal's span names, bounds-checked because the image
    /// is untrusted.
    fn blob(self: *const Image, off: u32, len: u32) EvalError![]const u8 {
        if (@as(usize, off) + @as(usize, len) > self.blob_len) return EvalError.Corrupt;
...
fn eval(img: *const Image, idx: u32, env: *Env, cur: u32, host: ?*const VehjeHost, session: ?*Session, arena: *varena.ValueArena) EvalError!Value {
    switch (try img.word(idx, 0)) {
        ...
        TAG_RAW => {
            ...
            var args: [ARG_CAP]VehjeOperand = undefined;
            var k: u32 = 0;
            ...
            var out: VehjeOperand = .{ .tag = SCALAR_UNIT, .payload = 0 };
            if (h.call(h.userdata, family, &args, args_len, &out) != 0) {
                return EvalError.HostFailed;
            }
            return host_boundary.fromOperand(out, session, arena);
        },
...
///
/// The runtime allocates nothing, so the scratch is the host's memory for the
/// session's lifetime. A single session is enough: the handle is per-call in
/// every current embedding, and a host wanting concurrency lends separate
/// regions.
var the_session: Session = .{ .scratch = &.{} };

export fn vehje_runtime_new(scratch: ?[*]u8, len: usize) ?*anyopaque {
    const p = scratch orelse return null;
    the_session = .{ .scratch = p[0..len], .used = 0 };
    return @ptrCast(&the_session);
}

export fn vehje_runtime_free(rt: ?*anyopaque) void {
    // Nothing to release: the scratch was never the runtime's memory.
    _ = rt;
}
...
/// the call returns.
fn marshal(v: Value, out: []u8, arena: *const varena.ValueArena) !usize {
    return switch (v) {
        ...
        .str => |b| value_image.writeScalar(out, .str, b),
        .compound => |root| varena.writeTree(out, arena, root),
...
pub export fn vehje_runtime_execute(
    ...
    const session: ?*Session = if (rt) |p| @ptrCast(@alignCast(p)) else null;
    var slots: [1024]Binding = undefined;
    // the value arena is lent exactly as the environment arena is: fixed
    // backing provisioned here, bump-allocated within, owned by nobody after.
    var vslots: [1024]varena.ValueNode = undefined;
    var vpool: [2048]u32 = undefined;
    var vblob: [8192]u8 = undefined;
    var arena = varena.ValueArena{ .slots = vslots[0..], .pool = vpool[0..], .blob = vblob[0..] };
    const v = evalImage(input[0..len], slots[0..], host, session, &arena) catch return VEHJE_RESULT_ERR;
    ...
    const n = marshal(v, buf[0..], &arena) catch return VEHJE_RESULT_ERR;
