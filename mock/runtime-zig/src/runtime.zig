...
fn toOperand(v: Value, arena: *const varena.ValueArena, session: ?*Session) EvalError!VehjeOperand {
    return switch (v) {
...
fn fromOperand(o: VehjeOperand, session: ?*Session, arena: ?*varena.ValueArena) EvalError!Value {
    return switch (o.tag) {
    ...
    switch (try img.word(idx, 0)) {
        ...
        TAG_RAW => {
            ...
            while (k < args_len) : (k += 1) {
                ...
                args[k] = try toOperand(v, arena, session);
            }
            ...
            return fromOperand(out, session, arena);
        },
