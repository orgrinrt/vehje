...
mod end_to_end {
    ...
    const WORD: usize = 4; // lint:allow(no-bare-numeric) lint:allow(arvo-types-only) reason: wire word size; tracked: #207
    const HEADER_WORDS: usize = 7; // lint:allow(no-bare-numeric) lint:allow(arvo-types-only) reason: wire header width; tracked: #207
    const NODE_WORDS: usize = 7; // lint:allow(no-bare-numeric) lint:allow(arvo-types-only) reason: wire node width; tracked: #207

    fn put(buf: &mut [u8], at: usize, w: u32) { // lint:allow(no-bare-numeric) lint:allow(arvo-types-only) reason: wire byte encoding; tracked: #207
    ...
    fn node_at(i: usize) -> usize { // lint:allow(no-bare-numeric) lint:allow(arvo-types-only) reason: wire offset arithmetic; tracked: #207
