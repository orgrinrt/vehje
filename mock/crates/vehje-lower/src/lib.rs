...
use vehje_ir::{hash_of, Arena, BinderDomain, Builder, Literal, Node, NodeRef, Span, StructuralHash};
...
const ARG_CAP: usize = 64; // lint:allow(no-bare-numeric) lint:allow(arvo-types-only) reason: fixed-cap ANF operand-arity scratch per compound; overflow returns Isnt; tracked: #38
...
impl Lower {
    ...
    pub fn lower(
        ...
        match strategy {
            LowerStrategy::Cheap => {
                // the redirecting passes read the arena and record redirects;
                // ANF then reads that folded/shared view and appends its nodes.
                MacroExpand.apply(builder.arena(), root, rw);
                if self.rules.const_fold.0 {
                    ConstFold.apply(builder.arena(), root, rw);
                }
                if self.rules.cse.0 {
                    Cse.apply(builder.arena(), root, rw, cse_table);
                }
                Anf.apply(builder, root, rw, binders)
            }
            ...
            LowerStrategy::Saturate => {
                self.lower(LowerStrategy::Cheap, builder, root, rw, cse_table, binders)
            }
        }
    }
}
