...
pub fn print(file_label: &str, src: &str, diag: &Diagnostic) {
    // lint:allow(bare_string) reason: host-side CLI uses &str for file path and source body; tracked: #73 lint:allow(no-bare-string) tracked: #207
...
fn line_col(src: &str, off: usize) -> (u32, u32) {
    // lint:allow(bare_numeric) lint:allow(bare_string) reason: 1-based line/col tuple is the conventional CLI diagnostic output shape; tracked: #73 lint:allow(arvo-types-only) lint:allow(no-bare-numeric) lint:allow(no-bare-string) tracked: #207
